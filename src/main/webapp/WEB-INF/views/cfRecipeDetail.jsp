...
<jsp:include page="inc/header.jsp" />
                ...
                <div class="message-box">
                    <div class="recipe-meta-info">
                        <div class="meta-row">
                            ...
                            <span class="meta-value">${recipe.cook_type}</span>
                        </div>
                        <div class="meta-row">
                            <span class="meta-label">난이도:</span>
                            <span class="meta-value">${recipe.recipe_difficulty}</span>
                        </div>
                        <c:if test="${recipe.cooking_time != null}">
                            <div class="meta-row">
                                <span class="meta-label">시간:</span>
                                <span class="meta-value">${recipe.cooking_time}분</span>
                            </div>
                        </c:if>
                        <c:if test="${recipe.servings != null}">
                            <div class="meta-row">
                                <span class="meta-label">인분:</span>
                                <span class="meta-value">${recipe.servings}인분</span>
                            </div>
                        </c:if>
                        <div class="meta-row view-count-row">
                            <span class="meta-label">조회수:</span>
                            <span class="meta-value">👁️ <fmt:formatNumber value="${recipe.view_count}" pattern="#,###" /></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="achievement-box">
                    ...
                    <c:if test="${not empty recipe.tags}">
                        <div class="recipe-tags">
                            <c:forEach var="tag" items="${recipe.tags.split(',')}" varStatus="status">
    ...
    <div class="comment-section">
        ...
        <c:if test="${currentUserIdx == null}">
            <div class="login-prompt">
                <p>댓글을 작성하려면 <a href="${cpath}/login">로그인</a>이 필요합니다.</p>
            </div>
        </c:if>
            ...
            <c:choose>
                <c:when test="${not empty reviews}">
                    <c:forEach var="review" items="${reviews}">
                        <div class="comment-item ${review.super_idx != null ? 'reply' : 'main'}" data-review-id="${review.review_idx}">
                            <div class="comment-content">
                                <div class="comment-author">
                                    <strong>${review.nick}</strong>
                                ...
                                <!-- 댓글 수정 폼 (숨김) -->
                                <div class="comment-edit-form" id="edit-form-${review.review_idx}" style="display: none;">
                                    <input type="text" class="edit-input" value="${review.cmt_content}" maxlength="500">
                                    <div class="edit-actions">
                                        <button onclick="saveEdit(${review.review_idx})" class="btn-save">저장</button>
                                        <button onclick="cancelEdit(${review.review_idx})" class="btn-cancel">취소</button>
                                    </div>
                                </div>
                                
                                <div class="comment-actions">
                                    <c:if test="${review.super_idx == null && currentUserIdx != null}">
                                        <button onclick="showReplyForm(${review.review_idx})" class="btn-reply">답글</button>
                                    </c:if>
                                    
                                    <!-- 작성자만 수정/삭제 가능 -->
                                    <c:if test="${currentUserIdx == review.user_idx}">
                                        <button onclick="editComment(${review.review_idx})" class="btn-edit">수정</button>
                                        <button onclick="deleteComment(${review.review_idx})" class="btn-delete">삭제</button>
                                    </c:if>
                                </div>
                                
                                <!-- 답글 작성 폼 (숨김) -->
                                <c:if test="${currentUserIdx != null && review.super_idx == null}">
                                    <div class="reply-form" id="reply-form-${review.review_idx}" style="display: none;">
                                        <input type="text" placeholder="답글을 입력하세요..." maxlength="500">
                                        <div class="reply-actions">
                                            <button onclick="submitReply(${review.review_idx})" class="btn-submit">답글 등록</button>
                                            <button onclick="hideReplyForm(${review.review_idx})" class="btn-cancel">취소</button>
                                        </div>
                                    </div>
                                </c:if>
                            </div>
        ...
        <c:if test="${totalPages > 1}">
            <div class="pagination">
                ...
                <c:forEach begin="1" end="${totalPages}" var="pageNum">
                    <a href="?page=${pageNum}" class="page-btn ${pageNum == currentPage ? 'active' : ''}">${pageNum}</a>
                </c:forEach>
    ...
    <script>
        ...
        function saveEdit(reviewIdx) {
            ...
            $.post('${cpath}/recipe/review/update', {
                reviewIdx: reviewIdx,
                cmtContent: content
            })
            .done(function(response) {
        ...
        function deleteComment(reviewIdx) {
            ...
            $.post('${cpath}/recipe/review/delete', {
                reviewIdx: reviewIdx
            })
            .done(function(response) {
                if (response.success) {
